...
- `CFMPGameMode`, Server Target, CFNetSmooth, 서버 런처, 네트워크 테스트 도구는 이번 세션에서 건드리지 않는다.
- 현재 착수 판단에서는 위 항목을 참고/보류 대상으로만 읽고, 실제 보존/삭제/수정 판단은 별도 작업으로 분리한다.
...
1. 싱글 실행 기준선 전환
2. 기준 차량 1대 주행감 고도화
3. 차량 카메라 / 로컬 조준 / Reticle 피드백 고도화
