...
타겟 선택과 락온은 같은 상태가 아니다.

```text
선택 대상
= 플레이어가 지속적으로 정보를 확인하고 장비 평가의 기준으로 삼는 대상

장비 락온
= 특정 무기 또는 장비가 거리, 각도, 시야, 시간과 자원 조건을 충족한 상태
```

선택 입력과 락온 입력이 반드시 별도로 존재할 필요는 없다. 기본 락온 방식은 선택된 타겟을 유효 조준 영역에 유지하면 자동으로 진행되는 구조로 시작한다. 다만 선택 상태와 락온 진행·완료 상태의 데이터 소유권은 반드시 분리한다.
...
락온 진행률은 센서 원본 정보가 아니라 개별 무기 또는 장비의 상태로 관리한다. 정확한 방어 수치와 내부 부품 상태는 가시 식별만으로 자동 공개하지 않고 센서 스캔 또는 별도 정보 조건을 요구한다.

---

## 4. 락온 기본 정의

락온은 선택된 목표를 일정 시간 장비의 유효 센서·조준 범위 안에 유지해서, 미사일, 터렛 보정, 약점 스캔, 전자전 같은 시스템을 사용할 수 있게 만드는 장비별 추적 상태다.
...
락온 획득 방식은 무기 또는 장비 데이터로 확장 가능해야 한다.
...
기본값은 `CrosshairHold` 또는 제한된 `TargetCone`으로 두고, 선택된 타겟을 유효 조준 영역에 유지하는 동안 진행도가 상승하도록 한다.
...
```text
01. 타겟 선택 시스템이 지속 선택 대상을 제공한다.
02. 무기 또는 장비가 선택 대상에 대해 락온 가능 조건을 평가한다.
03. 목표가 센서 범위와 유효 조준 영역 안에 있으면 락온 게이지가 상승한다.
04. 목표가 엄폐 / 연막 / 빠른 횡이동 / 전자전에 들어가면 게이지가 감소하거나 정지한다.
05. 락온 완료 시 해당 무기 또는 장비의 락온 기반 기능을 사용할 수 있다.
06. 목표가 락온 유지 조건을 벗어나면 락온이 약화되거나 끊긴다.
07. 락온이 끊겨도 타겟 선택은 별도 규칙에 따라 유지될 수 있다.
```
...
```text
01. 타겟 선택과 장비 락온 상태를 분리한다.
02. 선택된 목표가 일정 거리 안에 있어야 한다.
03. 기본 방식에서는 목표가 화면 조준 영역 또는 락온 원 안에 있어야 한다.
04. 일정 시간 유지하면 해당 장비의 락온이 완료된다.
05. 목표가 엄폐물 뒤로 가면 락온이 감소하거나 끊긴다.
06. 목표의 횡방향 속도가 높으면 락온 시간이 길어진다.
07. 락온 중에는 배터리를 조금씩 사용한다.
08. 락온 완료 후 경량 미사일을 발사할 수 있다.
09. 목표가 연막을 사용하면 락온이 약화되거나 끊긴다.
10. Sensor Contact와 최소 Radar 표시를 제공한다.
11. 선택 타겟의 미공개 정보는 ???로 유지한다.
12. 가시 식별과 센서 스캔의 정보 공개 경로를 분리한다.
```
...
```text
CarFight Lock-On / Sensor / EW v0.3.0
