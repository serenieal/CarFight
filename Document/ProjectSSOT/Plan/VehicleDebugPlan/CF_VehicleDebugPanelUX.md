...
## 20.1 Phase UX-1/2 C++ 기반 구현 결과

아래 C++ 기반 작업을 완료했다.

1. `FCFVehicleDebugNavItemViewData`와 `ECFVehicleDebugNavGroup`를 추가했다.
2. `FCFVehicleDebugSectionViewData`에 Navigation 그룹, 순서, 표시 여부, 배지 힌트를 추가했다.
3. `CFVehicleDebugPanelWidget`에 `SelectedSectionId`, `VerticalBox_NavHost`, `VerticalBox_SelectedSectionHost`, `NavItemWidgetClass`를 추가했다.
4. `SelectSectionById()`와 `GetSelectedSectionId()`를 추가했다.
5. `VerticalBox_SelectedSectionHost`와 `DynamicSectionWidgetClass`가 모두 유효할 때만 선택 Section 렌더링을 사용한다.
6. 선택 Section 경로가 준비되지 않으면 기존 `VerticalBox_DynamicSectionHost` 전체 Section fallback을 유지한다.
7. `CFVehicleDebugNavItemWidget` C++ 부모 클래스를 추가했다.
8. NavItem 클릭은 `RequestSelectThisNavItem()`으로 단일화했다.
9. C++ 버튼 바인딩이 동작하지 않는 경우 `WBP_VehicleDebugNavItem` 이벤트 그래프에서 `RequestSelectThisNavItem()`을 호출하는 fallback을 둔다.
10. Navigation Item은 개수가 같으면 재생성하지 않고 기존 위젯에 ViewData만 갱신한다.
11. `CarFight_ReEditor Win64 Development` 빌드가 성공했다.
12. Navigation 선택 상태는 문자열 접두사 대신 버튼 배경색과 텍스트 색으로 표시한다.

BP/UMG 수동 작업 결과:

1. `WBP_VehicleDebugPanel`에 `VerticalBox_NavHost`가 추가되었다.
2. `WBP_VehicleDebugPanel`에 `VerticalBox_SelectedSectionHost`가 추가되었다.
3. `WBP_VehicleDebugNavItem`이 생성되었다.
4. `WBP_VehicleDebugNavItem`은 `CFVehicleDebugNavItemWidget` 부모 클래스를 사용한다.
5. `WBP_VehicleDebugPanel`의 `NavItemWidgetClass`가 `WBP_VehicleDebugNavItem`을 가리킨다.
6. PIE에서 왼쪽 Navigation과 오른쪽 Selected Section 표시가 의도대로 동작하는 것을 확인했다.

남은 개선 후보:

1. Navigation 영역과 Content 영역의 폭 비율 조정
2. 선택 Section 내부 Header 중복 표시 여부 검토
...
### v0.2.1 - 2026-04-27
- Navigation 선택 상태를 `> Overview` 문자열 접두사가 아니라 버튼 배경색/텍스트 색으로 표시하는 기준으로 갱신했다.
- `CFVehicleDebugNavItemWidget`에서 선택/비선택/배지 색상 값을 조정할 수 있는 구조를 반영했다.
- `CarFight_ReEditor Win64 Development` 빌드 성공 결과를 기록했다.

### v0.2.0 - 2026-04-27
- Phase UX-1/2 C++ 기반 구현 결과를 문서에 반영했다.
- Navigation ViewData, Selected Section 상태, NavItem 위젯, 기존 DynamicHost fallback 유지 기준을 기록했다.
- 남은 BP/UMG 수동 연결 작업을 별도 체크리스트로 정리했다.
- `CarFight_ReEditor Win64 Development` 빌드 성공 결과를 기록했다.
- PIE에서 Navigation + Selected Section 표시가 의도대로 동작한 결과를 반영했다.
