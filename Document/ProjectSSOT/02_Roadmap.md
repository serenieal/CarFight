...
현재 병목은 서버 권한 전투가 아니라, 아래 여섯 가지다.

```text
1. 현재 Aim Fire 골격이 실제 차량 무기 시스템으로 닫히지 않았다.
2. 발사 성공/실패를 이펙트, 사운드, 조준 UI로 즉시 읽기 어렵다.
3. 피격 판정과 피해 처리가 없어 전투 결과가 남지 않는다.
4. 주행과 전투를 함께 반복 검증하는 테스트 루프가 부족하다.
5. 조작감, 전투 템포, 피드백 개선 기준이 아직 전투 루프에 묶이지 않았다.
6. 핵심 게임 루프 PASS / FAIL 기준이 아직 명확하지 않다.
```
...
3. **S1 — 싱글 실행 기준선 전환**
4. **C1 — 차량 무기 조준 및 발사 기능 구현**
5. **C2 — 발사 이펙트 / 사운드 / 조준 UI 구현**
6. **C3 — 피격 판정 및 피해 처리 구현**
7. **C4 — 주행과 전투 흐름 반복 테스트**
8. **C5 — 조작감 / 전투 템포 / 피드백 개선**
9. **C6 — 핵심 게임 루프 검증**
...
### 완료 조건
- [ ] PIE 1인 플레이에서 기준 차량이 조준 방향으로 발사 요청을 만든다.
- [ ] 발사 성공 / 발사 불가 / 쿨다운 상태를 구분할 수 있다.
- [ ] 발사 원점, 발사 방향, 현재 조준 상태를 디버그 또는 UI로 확인할 수 있다.
- [ ] 서버 창, 2클라 창, 네트워크 접속 없이 검증 가능하다.

---

## C2 — 발사 이펙트 / 사운드 / 조준 UI 구현
### 목표
플레이어가 발사 성공, 발사 불가, 조준 상태를 즉시 이해하게 만든다.

### 범위
- 발사 피드백은 판정 흐름을 읽히게 하는 최소 수준으로 시작한다.
- 최종 VFX/SFX 품질, 대량 무기별 연출, 복잡한 HUD는 현재 범위에서 제외한다.

### 해야 할 일
- 발사 성공 시 최소 이펙트와 사운드 호출 기준을 정한다.
- 발사 불가 또는 쿨다운 상태를 조준 UI에서 구분한다.
- Reticle이 로컬 조준 상태와 발사 가능 상태를 함께 보여줄지 결정한다.
- UI 텍스트는 한국어 표시 정책을 따른다.

### 완료 조건
- [ ] 발사 성공 시 화면/소리 피드백이 즉시 발생한다.
- [ ] 발사 불가 또는 쿨다운 상태가 조준 UI로 구분된다.
- [ ] 발사 피드백이 피해 판정과 충돌하지 않고 분리되어 있다.
- [ ] 피드백이 없어도 판정이 되고, 판정이 없어도 피드백만 있는 상태가 되지 않게 분리 기준이 기록된다.
...
### 완료 조건
- [ ] 발사 결과가 목표물 또는 차량에 피격으로 기록된다.
- [ ] 피해량이 누적되고 현재 체력 또는 파괴 상태를 확인할 수 있다.
- [ ] 빗나감 / 피격 / 피해 적용 실패를 구분할 수 있다.
- [ ] 서버 없이 로컬 테스트에서 반복 검증 가능하다.

---

## C4 — 주행과 전투 흐름 반복 테스트
### 목표
주행, 조준, 발사, 피격, 피해가 한 흐름으로 반복되는지 확인한다.
...
- 휠 시각 품질, 카메라 흔들림, Reticle 가독성을 전투 테스트 중 함께 관찰한다.
- 실패를 Core / Data / Asset / Input / UI / Quality / DocMismatch로 분류한다.

### 완료 조건
- [ ] 주행 중 조준/발사 입력이 끊기지 않는다.
- [ ] 같은 루프를 5회 이상 반복해도 입력, UI, 판정 상태가 꼬이지 않는다.
- [ ] 조작감 문제와 전투 판정 문제를 분리해 기록할 수 있다.
