...
- 핵심 기능 구현: **완료**
- 실제 PIE 동작 확인: **완료**
- 전체 계획 기준 진행도: **약 85~90%**
- 남은 작업 성격: **기능 추가보다 안정화 / 충돌 방지 / 검증 정리 중심**
...
- [x] 키보드/게임패드 공용 2D 입력 해석 구조가 들어가 있다.
...
- [ ] `VehicleMove` 활성 시 기존 `Throttle / Steering / Brake` 단일 축 입력을 코드 레벨에서 명확히 무시한다.
- [x] 현재는 IMC 설정으로 충돌을 회피할 수 있다.
- [ ] 코드 자체가 중복 입력에 대해 항상 안전하다고 보기 어렵다.

### 판단
현재 상태는 **작동은 하지만 IMC 의존도가 남아 있는 상태**다.
즉, 프로젝트 규모가 커지면 다시 충돌 가능성이 있다.
...
## 3-4. 작업 산출물 정리
- [ ] 수정된 C++ 파일 changelog를 별도 정리했다.
- [ ] IMC 변경 요약을 별도 정리했다.
- [ ] PIE 검증 캡처 또는 테스트 로그를 폴더 산출물로 남겼다.
- [ ] 남은 이슈 목록을 별도 문서로 정리했다.

### 판단
기능은 들어갔지만, **증빙/정리 산출물은 아직 부족**하다.
...
### Priority 1. 입력 충돌 방지
- [ ] `VehicleMove` 사용 시 기존 축 입력과의 중복 적용 방지 정책을 코드 레벨로 정리한다.

### Priority 2. 회귀 검증 정리
- [ ] 장치 모드별 검증 결과를 체크리스트나 테스트 로그로 남긴다.
...
- 현재는 IMC 세팅에 의존하는 부분이 일부 남아 있다.
- 코드 레벨 충돌 방지까지 끝난 상태는 아니다.
