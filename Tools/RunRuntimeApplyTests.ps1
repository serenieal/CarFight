...
#
# File: RunRuntimeApplyTests.ps1
# Version: v1.0.0
# Date: 2026-09-02
# Description: CF-FQ-041 Runtime Apply focused Automation을 공식 UE 5.8 Editor에서 독립 로그로 실행합니다.
# Scope: RuntimeApply test filter만 실행하며 Product Source/Config/Content Asset을 저장하지 않습니다.
# Changelog:
# - v1.0.0: DataAuthoring Runner와 로그 파일을 공유하지 않는 RuntimeApply 전용 unattended Automation 진입점을 추가.
# Migration:
# - 결과는 UE/Saved/CarFight/RuntimeApplyAutomationResult.json과 CFRuntimeApplyAutomation.log에만 기록합니다.

[CmdletBinding()]
param(
    # 실행할 Runtime Apply Automation 전체 경로 prefix 또는 filter 문자열입니다.
    [string]$TestFilter = 'CarFight.RuntimeApply'
)
...
# Runtime Apply 전용 Unreal 로그입니다.
$AutomationLogPath = Join-Path $RepositoryRoot 'UE\Saved\Logs\CFRuntimeApplyAutomation.log'

# Browser가 bounded evidence로 회수할 Runtime Apply 전용 JSON 결과 파일입니다.
$ResultJsonPath = Join-Path $RepositoryRoot 'UE\Saved\CarFight\RuntimeApplyAutomationResult.json'
...
# 공식 Editor 실행 파일에 전달할 bounded unattended Automation 인자 목록입니다.
$EditorArguments = @(
    $ProjectFile,
    '-unattended',
    '-nop4',
    ...
    '-stdout',
    '-FullStdOutLogOutput',
    '-LogCmds="LogModelContextProtocol off"'
)

# exact UnrealEditor Automation PID를 시작하고 해당 PID만 종료까지 기다립니다.
...
if (-not (Test-Path -LiteralPath $AutomationLogPath -PathType Leaf)) {
    throw "Runtime Apply Automation 로그가 생성되지 않았습니다. EngineExitCode=$EngineExitCode"
}
...
# 결과 marker에서 허용할 Runtime Apply test path prefix의 정규식 안전 문자열입니다.
...
$Result = [ordered]@{
    schema_version = 'carfight_runtime_apply_automation_result_v1'
...
if (-not $AutomationPassed) {
    # 실패 시 Runtime Apply 관련 진단 로그만 bounded하게 출력합니다.
    $DiagnosticLines = @($AutomationLogText -split "`r?`n" | Where-Object {
        $_ -match 'LogAutomation|Test Completed|CarFight\.RuntimeApply|Error:'
