...
// Migration:
// - `CarFight.VehicleDAWizard` tab identity와 hidden spawner는 DEL1~DEL7 판정 전까지 유지한다.
...
// Editor 모듈 로드 시 Guided Builder, Vehicle Authoring Workspace와 기존 Vehicle DA Wizard 탭/메뉴를 등록합니다.
void FCarFightReEditorModule::StartupModule()
{
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
		...
		.SetDisplayName(LOCTEXT("VDAWizardTabTitle", "Legacy Vehicle DA Wizard"))
		.SetMenuType(ETabSpawnerMenuType::Hidden);
