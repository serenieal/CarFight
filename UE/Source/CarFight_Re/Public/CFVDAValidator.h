...
public:
	...
	UFUNCTION(BlueprintCallable, Category="CarFight|Vehicle DA Validator", meta=(DisplayName="피팅 질량 설정 검사 (Validate Fitting Mass Config)", ToolTip="BaseVehicleMassKg와 MaximumGrossMassKg가 둘 다 미설정인지, 둘 다 유효하게 설정됐는지와 최대 총중량이 기준 질량 이상인지 검사합니다."))
	static FCFVDAValidationReport ValidateFittingMassConfig(UCFVehicleData* TargetVehicleData);

	// [v1.3.0] 대상 VehicleData의 MountProfile 안정 ID와 Hardpoint 위치 참조 정합성을 검사합니다.
	UFUNCTION(BlueprintCallable, Category="CarFight|Vehicle DA Validator", meta=(DisplayName="장착 프로파일 검사 (Validate Mount Profiles)", ToolTip="MountProfiles의 MountProfileId 중복·누락과 LocationSlotRef가 실제 HardpointSlots를 참조하는지 검사합니다. MountProfiles 빈 배열 자체는 오류가 아닙니다."))
	static FCFVDAValidationReport ValidateMountProfiles(UCFVehicleData* TargetVehicleData);

	// 대상 VehicleData의 Movement 수치 기본 범위와 위험값을 검사합니다.
	UFUNCTION(BlueprintCallable, Category="CarFight|Vehicle DA Validator", meta=(DisplayName="Movement 설정 검사 (Validate Movement Config)", ToolTip="대상 차량 DA의 VehicleMovementConfig 값 중 실제 적용 경로와 맞지 않는 설정, 테스트 전 확인해야 할 위험 수치와 기본 범위 오류를 검사합니다."))
	static FCFVDAValidationReport ValidateMovementConfig(UCFVehicleData* TargetVehicleData);

			// 대상 VehicleData의 WheelVisual 예상 휠 개수, 조향 전륜 개수와 자동 스케일 안전 범위를 검사합니다.
	UFUNCTION(BlueprintCallable, Category="CarFight|Vehicle DA Validator", meta=(DisplayName="WheelVisual 설정 검사 (Validate Wheel Visual Config)", ToolTip="대상 차량 DA의 WheelVisualConfig에서 예상 휠 개수, 조향 전륜 개수와 WheelRadius 기반 자동 스케일 최소·최대값이 유효한지 검사합니다."))
